...
void Valiant::setup(HardwareSerial * s) {
	...
	NewPing(ultrasonicTrigger, ultrasonicEcho);
...
void Valiant::turretMove(int degree) {
	turret.write(degree);
}
...
void Valiant::forwardsspeed(int speed) {
	#ifdef DEBUG_MOTORS
	if (DEBUG_MOTORS) {
...
void Valiant::reverse(int speed) {
	#ifdef DEBUG_MOTORS
	if (DEBUG_MOTORS) {
...
/*
 * NewPing Library added to Valiant Library to reduce dependancies
*/
Valiant::NewPing(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance) {
#if DO_BITWISE == true
...
unsigned int Valiant::ping_ultrasonic(unsigned int max_cm_distance) {
	if (max_cm_distance > 0) set_max_distance(max_cm_distance); // Call function to set a new max sensor distance.

	if (!ping_ultrasonic_trigger()) return NO_ECHO; // Trigger a ping, if it returns false, return NO_ECHO to the calling function.
	...
	return (micros() - (_max_time - _maxEchoTime) - PING_ULTRASONIC_OVERHEAD); // Calculate ping time, include overhead.
}


unsigned long Valiant::ping_ultrasonic_cm(unsigned int max_cm_distance) {
	unsigned long echoTime = Valiant::ping_ultrasonic(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
...
unsigned long Valiant::ping_ultrasonic_in(unsigned int max_cm_distance) {
	unsigned long echoTime = Valiant::ping_ultrasonic(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
...
unsigned long Valiant::ping_ultrasonic_median(uint8_t it, unsigned int max_cm_distance) {
	...
	while (i < it) {
		...
		last = ping_ultrasonic(max_cm_distance);  // Send ping.
		...
		if (i < it && micros() - t < PING_ULTRASONIC_MEDIAN_DELAY)
			delay((PING_ULTRASONIC_MEDIAN_DELAY + t - micros()) / 1000); // Millisecond delay between pings.
...
boolean Valiant::ping_ultrasonic_trigger() {
#if DO_BITWISE == true
...
void Valiant::set_max_distance(unsigned int max_cm_distance) {
#if ROUNDING_ENABLED == false
	...
	void Valiant::ping_ultrasonic_timer(void (*userFunc)(void), unsigned int max_cm_distance) {
		if (max_cm_distance > 0) set_max_distance(max_cm_distance); // Call function to set a new max sensor distance.

		if (!ping_ultrasonic_trigger()) return;         // Trigger a ping, if it returns false, return without starting the echo timer.
		timer_us(ECHO_TIMER_FREQ, userFunc); // Set ping echo timer check every ECHO_TIMER_FREQ uS.
	}


	boolean Valiant::check_timer() {
	...
	#endif
			timer_stop();                // Disable timer interrupt
			ping_ultrasonic_result = (micros() - (_max_time - _maxEchoTime) - PING_ULTRASONIC_TIMER_OVERHEAD); // Calculate ping time including overhead.
	...
	void Valiant::timer_us(unsigned int frequency, void (*userFunc)(void)) {
	...
	void Valiant::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
		intFunc = Valiant::timer_ms_cntdwn;  // Timer events are sent here once every ms till user's frequency is reached.
		...
		TIMSK4 = (1<<TOIE4);   // Enable Timer4 interrupt.
	#elif defined (__arm__) && defined (TEENSYDUINO)        // Timer for Teensy 3.x
		itimer.begin(Valiant::timer_ms_cntdwn, 1000);       // Set timer to 1ms (1000 uS).
	#elif defined (__arm__) && defined (PARTICLE)           // Timer for Particle
		itimer.begin(Valiant::timer_ms_cntdwn, 1000, uSec); // Set timer to 1ms (1000 uS).
	#else
	...
	void Valiant::timer_stop() { // Disable timer interrupt.
	#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
		TIMSK4 = 0;
...
unsigned int Valiant::convert_cm(unsigned int echoTime) {
#if ROUNDING_ENABLED == false
...
unsigned int Valiant::convert_in(unsigned int echoTime) {
#if ROUNDING_ENABLED == false
